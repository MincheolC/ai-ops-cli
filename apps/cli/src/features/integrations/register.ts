...
export const registerIntegrationCommands = (program: Command): void => {
  ...
  command
    ...
    .action((integrationId, opts: { command?: string; commandWindows?: string }) =>
      integrationInstallCommand(integrationId, opts),
    );
  command
  ...
  command
    ...
    .command('post-tool-use [integrationId]', { hidden: true })
    .description('Codex PostToolUse integration hook entrypoint')
    .option('--workflows <workflows>', 'comma-separated ai-ops PostToolUse workflows')
    .action((integrationId, opts: { workflows?: string }) =>
      integrationPostToolUseHookCommand({ legacyIntegrationId: integrationId, workflows: opts.workflows }),
    );
};
