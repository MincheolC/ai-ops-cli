...
- **프로젝트 로컬 설치 (Project-only):** 모든 설치/업데이트/삭제는 현재 프로젝트 디렉토리 기준으로 수행
...
### 6.2. Project-only 스코프 운영 원칙

`ai-ops`는 project-only 설치를 기본 정책으로 채택합니다.

**운영 원칙:**

- 설치 대상 기준 경로는 항상 `process.cwd()`입니다.
- 규칙 파일과 manifest는 프로젝트 내부에만 생성/갱신됩니다.
- CLI 명령(`init/update/diff/uninstall`)은 모두 동일한 project 기준 경로에서 동작합니다.
