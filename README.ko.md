...
`safe-local`은 `~/.codex/config.toml`에 `ai-ops-safe-local` user-level Codex permission profile을 upsert합니다. `~/.personal-project-contexts`, `${AI_OPS_HOME:-$HOME}/.ai-ops/context-promotion`, active project root 아래 `.codex/plans`에는 write를 허용하고, `.git`은 read-only로 두며 `**/*.env` read는 차단합니다. `PermissionRequest` hook이나 command allow rule은 설치하지 않습니다.
